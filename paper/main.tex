...
89 modèles), selon six axes complémentaires : (i)~dynamique temporelle de la diversité
stylistique (R1/R1-bis) ; (ii)~évolution mensuelle du \emph{style premium} (R2bis) ;
...
La diversité \emph{globale} augmente ($\beta = +0{,}057$ par mois en R1 brut,
$p < 0{,}001$), mais R1-bis montre que, sur une \textbf{cohorte fixe} de 9 modèles
présents $\geq 12$ mois (dont Llama-3.3-70B et Llama-3.1-8B), la dispersion
stylistique densifiée est \textbf{stable} ($\beta = -0{,}003$, $p = 0{,}71$) ---
compatible avec un \emph{artefact de composition} de l'arène plutôt qu'une
divergence stylistique réelle. Pas de dérive temporelle du premium (gras
$-1{,}4$ points/mois).
...
\begin{enumerate}[noitemsep]
  \item une dimension \textbf{temporelle} (R1, R1-bis, R2bis) absente du papier original ;
...
\subsection{R1-bis --- Cohorte stable et artefact de composition}

\subsubsection{Motivation}

Une hausse de $D_t$ peut refléter l'entrée de modèles stylistiquement distincts
(\emph{artefact de composition}) plutôt qu'une divergence des modèles existants.
R1-bis remesure la dispersion sur une \textbf{cohorte fixe}.

\subsubsection{Périmètre et densification}

Battles avec \texttt{source == "vote"}, ties exclus, timestamps valides
($<1\,\%$ NaT). Features densifiées :
\begin{equation}
  f^{\mathrm{dens}} = \frac{f_{\mathrm{brut}}}{\texttt{assistant\_chars}}
\end{equation}
(merge \texttt{votes\_length.parquet} sur \texttt{conversation\_pair\_id}).

\subsubsection{Cohorte stable}

Modèles retenus si $\geq 100$ battles totales et $\geq 12$ mois distincts
(fallback 10 mois si $<6$ modèles ; non déclenché). Neuf modèles retenus,
dont \texttt{llama-3.3-70b} et \texttt{llama-3.1-8b}.

\subsubsection{Métrique commune}

Distance euclidienne moyenne entre centroïdes (\texttt{method="centroid"},
identique à R1), z-score global, $\geq 100$ réponses/modèle/mois, skip si
$<5$ modèles. Pour chaque mois~$t$ :
\begin{align}
  D^{\mathrm{glob}}_t &= \mathrm{mean\_pdist}(\text{centroïdes de tous modèles actifs}), \\
  D^{\mathrm{coh}}_t &= \mathrm{mean\_pdist}(\text{centroïdes} \in \text{cohorte stable}).
\end{align}
...
\subsection{R1 --- Diversité globale et artefact de composition}

\subsubsection{R1 brut}

La diversité $D_t$ (comptes bruts) \textbf{augmente} (Figure~\ref{fig:r1}) :
$\hat{\beta} = +0{,}057$ par mois, $p < 0{,}001$. Corrélation forte avec
\texttt{n\_models} ($r = 0{,}83$) : l'expansion du parc (14 $\rightarrow$ 43 modèles)
explique en partie la courbe.

\begin{figure}[H]
  \centering
  \maybeincludegraphics[width=0.85\textwidth]{figures/R1_convergence_robustness.png}
  \caption{Diversité stylistique mensuelle (R1 brut, comptes bruts).}
  \label{fig:r1}
\end{figure}

\subsubsection{R1-bis --- cohorte stable}

Sur 79\,075 battles votées (features \textbf{densifiées}), fenêtre fév.\,2025 --
janv.\,2026 (12 mois), cohorte de \textbf{9 modèles} (Table~\ref{tab:cohorte}).

\begin{table}[H]
\centering
\caption{Cohorte stable R1-bis ($\geq 100$ battles, $\geq 12$ mois).}
\label{tab:cohorte}
\small
\begin{tabular}{lrr}
\toprule
...
\begin{figure}[H]
  \centering
  \maybeincludegraphics[width=0.85\textwidth]{figures/R1bis_cohorte_stable.png}
  \caption{R1-bis : dispersion globale vs cohorte stable (features densifiées).
  Annotations = \texttt{n\_models\_cohorte}.}
  \label{fig:r1bis}
\end{figure}

OLS sur \texttt{month\_idx} (Table~\ref{tab:r1bis_ols}) :

\begin{table}[H]
\centering
\caption{R1-bis : régressions OLS ($N = 12$ mois).}
\label{tab:r1bis_ols}
\begin{tabular}{lrrrr}
\toprule
Série & Pente & IC 95\,\% & $p$ & $R^2$ \\
\midrule
Dispersion globale & $+0{,}038$ & $[+0{,}015,\ +0{,}062]$ & 0{,}005 & 0{,}567 \\
Dispersion cohorte & $-0{,}003$ & $[-0{,}022,\ +0{,}016]$ & 0{,}71 & 0{,}015 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Lecture.} Globalement, la dispersion \emph{densifiée} monte encore
(nouveaux modèles chaque mois). Sur la cohorte fixe, la pente est nulle :
\textbf{pas de convergence} ni de divergence stylistique intra-cohorte.
Le stade~1 Goodhart (convergence vers un format unique) n'est pas observé ;
la hausse R1 globale est largement un \emph{artefact de composition}.
...
\midrule
R1 & Convergence globale ? & Brut $\uparrow$ ; cohorte stable $\approx$ flat & Stade 1 : non \\
R1-bis & Artefact composition ? & Globale $+0{,}038$* ; cohorte $-0{,}003$ n.s. & Composition \\
...
\begin{enumerate}
  \item \textbf{Pas de convergence intra-cohorte} (R1-bis) : sur 9 modèles stables,
        la dispersion densifiée est plate ($p = 0{,}71$) ; la hausse globale R1
        reflète surtout l'élargissement du parc.
...
\begin{enumerate}
  ...
  \item Monitorer $D_t$ \emph{global et cohorte stable} et $\gamma_{f,t}$ mensuellement ;
...
\begin{itemize}
  ...
  \item \textbf{R1-bis} : échelle densifiée $\neq$ R1 historique (comptes bruts) ;
        fenêtre 12 mois (2025-02 -- 2026-01) ; cohorte $n=9$.
\end{itemize}
...
Les données \textbf{ne montrent pas} une convergence stylistique sur une cohorte
fixe (R1-bis) ni une dérive temporelle du style premium (R2bis) ; elles \textbf{montrent} une sensibilité persistante
...
\begin{itemize}[noitemsep]
  \item \texttt{r1\_robustness.py}, \texttt{r1bis\_cohorte\_stable.py}, \texttt{r2bis\_style\_premium.py}
